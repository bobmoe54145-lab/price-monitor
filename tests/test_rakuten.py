...
def test_keyword_with_one_char_term_is_rejected_before_calling_api():
    client, session, _ = make_client([])
    with pytest.raises(ValueError, match="2文字以上"):
        client.search(keyword="JBL GO 3 スピーカー")
...
def test_pagination_stops_at_page_count_and_dedups():
    ...
    offers = client.search(keyword="xx", max_pages=5)
...
def test_not_found_404_returns_empty():
    ...
    assert client.search(keyword="xx") == []
...
def test_retries_429_then_succeeds():
    ...
    assert len(client.search(keyword="xx")) == 2
    assert len(session.calls) == 2
    assert 3 in sleeps
...
def test_throttles_between_requests():
    ...
    client.search(keyword="xx", max_pages=2)
